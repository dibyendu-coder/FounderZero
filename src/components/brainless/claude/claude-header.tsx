...
export function ClaudeHeader({
  ...
  tips = ["Ask Copilot to analyze your metrics, validate ideas, or plan weekly sprints"],
  whatsNew = [
    "Added Claude Code terminal theme UI",
  ...
  return (
    <fieldset
      className={cn(
        "min-w-0 rounded-[6px] border px-3 pb-3.5 pt-1 font-mono text-[13px] leading-[1.5] text-[#c0caf5] sm:px-4 bg-[#08080a]",
        className,
      )}
      style={{ borderColor: ROSE }}
    >
      <legend className="max-w-full truncate px-2 font-bold" style={{ color: ROSE }}>
      ...
      <div className="grid min-w-0 gap-4 sm:grid-cols-[minmax(0,1fr)_1px_minmax(0,1.1fr)]">
        ...
        <div className="min-w-0 space-y-1">
          ...
          {tips.map((t) => (
            <div key={t} className="truncate text-slate-300">
              {t}
            </div>
          ))}
