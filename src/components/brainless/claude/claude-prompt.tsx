...
export function ClaudePrompt({
  ...
  const e = effort === false ? null : (EFFORTS[effort] || EFFORTS.xhigh);
  ...
  return (
    <div className={cn("min-w-0 font-mono text-[13px] leading-[1.6]", className)}>
      ...
      >
        <span aria-hidden className="shrink-0 pl-0 pr-0 font-bold" style={{ color: "#cd694a" }}>
          ❯
        </span>
        <input
          type="text"
          ...
          className={cn(
            "term-input min-w-0 flex-1 bg-transparent py-0.5 pl-[1ch] outline-none placeholder:text-[#565f89] font-mono",
            inputClassName,
          )}
          style={{ color: FG, caretColor: FG } as React.CSSProperties}
        />
      </div>
